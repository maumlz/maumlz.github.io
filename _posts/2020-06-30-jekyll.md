...
Lenguaje de plantillas que maneja Jekyll, cuenta con tres partes principales: `Objects`, `tags` y `filters`.
	...
	- Tags: crean la lógica y controlan el flujo de las plantillas. Sintaxis: {% raw %}`{%` y `%}`{% endraw %}.^[https://jekyllrb.com/docs/liquid/tags/]
	- Filtros: cambian el output de un Objeto. Sintaxis: {% raw %}`{{ "output" | " filtro" }}`{% endraw %}.^[https://jekyllrb.com/docs/liquid/filters/]
3. Front Matter:
...
Utiliza una variable especial {% raw %}`{{ content }}`{% endraw %} la cual tiene el contenido de la pagina que lo llama.
5. Includes:
